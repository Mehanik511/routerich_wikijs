...
- С помощью приложения **Терминал**.
- Через **SSH**, выполнив команду:
...
> Использование WPS может сделать сеть уязвимой для атак методом перебора пароля. Рекомендуется вместо этого использовать ручной ввод пароля для более надёжной защиты.
{.is-danger}
